...
public class Ind_Baskets_IndDeltaNoBaskets_Chart extends MyChartCreator {
	
	// Constructor
	public Ind_Baskets_IndDeltaNoBaskets_Chart(ApiObject apiObject) {
		super(apiObject);
	}
	...
	public void createChart() throws CloneNotSupportedException {
		...
		// Index
		MyTimeSeries deltaSerie = new MyTimeSeries("Ind delta no baskets", Themes.LIGHT_BLUE_3, 1.5f, bottomChartProps,
				apiObject.getStocksHandler().getIndDeltaNoBasketsList()) {
			/**
					 * 
					 */
			...
			@Override
			public double getData() {
				return apiObject.getStocksHandler().getDeltaNoBaskets();
			}
		};
		...
		// ----- Container ----- //
		MyChartContainer chartContainer = new MyChartContainer(charts, "Ind baskets indDelta no baskets Chart");
